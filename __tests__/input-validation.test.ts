    ...
    getInputMock.mockImplementation(name => {
        switch (name) {
            case 'validation-script':
                const fileBuffer = fs.readFileSync('__tests__/validation-script.yml', 'utf8');
...
describe('input validation', () => {
    ...
    it('provide a negative result if at least one input is not valid', async () => {
        ...
        const renderedItems = [];
        renderedItems.push("- Input : 'input-b' has to match the regex 'foo\\.bar.*', but found 'foooooo.bar-1234'");
        renderedItems.push("- Input : 'milliseconds' has to be a number, but found 'not a number'");
        renderedItems.push("- Input : 'time-to-live' has to be a number greater than 0, but found '-20'");
        renderedItems.push("- Input : 'enabled' has to be a boolean, but found 'oui'");
